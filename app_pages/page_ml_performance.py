...
def page_ML_performance_body():
    ...
    features_importance = plt.imread(
        f"outputs/ml_pipeline/predict_readmission/{version}/features_importance.png"
        )
    
    st.write(f"### ML Pipeline: Binary Classification Model\n\n")
    
    st.info(
        f"The model is trained to predict whether a patient will be readmitted or not "
        f"following a hospital visit.\n"
