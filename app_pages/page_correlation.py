...
def heatmap_pps(df, threshold, figsize=(20, 12), font_annot=8):
    ...
    if len(pps_matrix.columns) > 1:
        ...
        ax = sns.heatmap(
            pps_matrix, annot=True, xticklabels=True, yticklabels=True,
...
def page_correlation_body():
    ...
    df_correlation = load_hospital_dataset(
        "HospitalReadmissionsCorrelation.csv")
    ...
    st.info(
        f"**Business requirement 1:**  Data Visualization and Correlation "
        f"study\n"
        f" - We need to perform a correlation study to determine which "
        f"features correlate most closely to the target.\n"
        f" - A Pearson's correlation will indicate linear relationships "
        f"between numerical variables.\n"
        f" - A Spearman's correlation will measure the monotonic relationships"
        f" between variables.\n"
        f" - A Predicitve Power Score can also be used to determine "
        f"relationships between the variables and if they have any predictive "
        f"power against the target variable.\n"
    )
    ...
    st.write(
        ...
        f"understand how the feature variables are correlated and try to "
        f"identify a pattern which will possibly "
        f"help in the prediction of the target variable and to try to answer "
        f"Business Requirement 1.\n"
        f"* After performing Spearman and Pearson correlation, the following "
        f"variables are the most correlated: **{correlated_features}**\n"
        f"* The correlation study, after applying the Log Transformer, "
        f"Scaling and PCA, revealed moderate correlation between the feature "
        f"variables and the target variable.\n"
        f"* Also, the PPS score was calculated to determine the predictive "
        f"power of the variables,which show strong predictive power between "
        f"the target variable and some features.\n"
    )
    ...
    st.write(
        ...
        f"Below we are plotting the most correlated features with the target "
        f"variable in a countplot and also we are using a parallel plot to "
        f"better visualize the  pattern.\n\n"
        f"Also, to avoid clutter and give better understanding of the "
        f"features and their values, we have created the following bins for "
        f"the Parallel Plot:\n"
        ...
        f" * **n_lab_procedures** - 'up to 10', '11-50', '51-100', "
        f"'above 100'\n"
    )
    ...
    st.success(
        f"From the above correlation study and and by plotting the most "
        f"correlated features we notice a pattern for the readmitted "
        f"patients.\n"
        ...
        f"* A readmitted patient typically has between no inpatient visits "
        f"and 5 inpatient visits.\n"
