...
def hypothesis_1():
  ...
  df_parallel[['n_lab_procedures_bins','readmitted']].value_counts(normalize=True).mul(100).round(2).astype(str)
  
  fig,ax = plt.subplots(figsize=(12,6))enumerate()
  ax = sns.countplot(df_parallel, x=df_parallel['n_lab_procedures_bins'], hue=df_parallel['readmitted'])
  st.pyplot(fig)
...
def hypothesis_2():
  ...
  ax = sns.barplot(df, x='change', y='readmitted', hue='diabetes_med')
  st.pyplot(fig)
...
def hypothesis_3():
  ...
  fig,ax = plt.subplots(figsize=(12,6))
    ax = sns.countplot(df, x=df['age'], hue=df_parallel['readmitted'])
    st.pyplot(fig)
